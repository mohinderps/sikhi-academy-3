...
const saakhis = [
  ...
  { 
    ...
    content: "ਇੱਕ ਵਾਰ ਗੁਰੂ ਗੋਬਿੰਦ ਸਿੰਘ ਜੀ ਦੇ ਦਰਬਾਰ ਵਿੱਚ ਇੱਕ ਰਾਜਾ ਆਇਆ। ਉਹ ਆਪਣੇ ਰਾਜ ਦੀ ਸ਼ਾਨ ਵਿੱਚ ਬਹੁਤ ਘੁਮੰਡੀ ਸੀ। ਗੁਰੂ ਜੀ ਨੇ ਉਸਨੂੰ ਇੱਕ ਕਟੋਰਾ ਦਿੱਤਾ ਅਤੇ ਕਿਹਾ, 'ਇਸ ਨੂੰ ਪਾਣੀ ਨਾਲ ਭਰ ਕੇ ਲਿਆਓ, ਪਰ ਇੱਕ ਸ਼ਰਤ ਹੈ - ਪਾਣੀ ਡੁੱਲ੍ਹਣਾ ਨਹੀਂ ਚਾਹੀਦਾ।' ਰਾਜੇ ਨੇ ਬਹੁਤ ਕੋਸ਼ਿਸ਼ ਕੀਤੀ ਪਰ ਉਹ ਅਸਫਲ ਰਿਹ"
  }
]
...
export function SaakhiListing() {
  ...
  const filteredSaakhis = saakhis.filter(saakhi => 
    saakhi.title.toLowerCase().includes(searchTerm.toLowerCase()) &&
    (filterGuruJi === '' || saakhi.guruJi === filterGuruJi)
  )

  const uniqueGuruJis = Array.from(new Set(saakhis.map(saakhi => saakhi.guruJi)))
  ...
  return (
    <div className="min-h-screen bg-orange-50 flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSaakhis.map(saakhi => (
            <Card key={saakhi.id} className="hover:shadow-lg transition-shadow duration-300">
              <CardHeader>
                <CardTitle className="text-xl font-semibold text-orange-700">{saakhi.title}</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <footer className="bg-orange-100 text-center p-4">
        <p className="text-gray-600">&copy; 2024 Sikhi Academy. All rights reserved.</p>
      </footer>
    </div>
  )
}